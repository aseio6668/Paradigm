...
use paradigm_core::genesis::{GenesisManager, GenesisConfig};
use paradigm_core::api::{ApiState, start_api_server, generate_sample_tasks};
...
async fn main() -> Result<()> {
    ...
    if !addnodefile_path.is_empty() {
        match load_peers_from_file(addnodefile_path, port) {
            Ok(file_peers) => {
                ...
                tracing::info!("Loaded {} additional peers from file: {}", 
    ...
    if is_genesis {
        ...
        genesis_manager.initialize_chain_from_genesis(&genesis_block).await?;
        
        tracing::info!("✅ Genesis blockchain initialized successfully!");
        tracing::info!("Network treasury holds {} PAR tokens", 
    ...
    if enable_api {
        ...
        tracing::info!("Tasks endpoint: http://localhost:{}/api/tasks/available", api_port);
    }
    ...
    let status_task = tokio::spawn(async move {
        ...
        loop {
            ...
            tracing::info!("Network sync: {} {}", sync_info.status_string(), sync_info.progress_bar());
        }
    });
...
fn load_peers_from_file(file_path: &str, default_port: u16) -> Result<Vec<String>> {
    ...
    for (line_num, line) in content.lines().enumerate() {
        ...
        // Validate the address format
        if let Err(_) = peer_address.parse::<std::net::SocketAddr>() {
            tracing::warn!("Invalid peer address format at line {}: {}, skipping", line_num + 1, peer_address);
            continue;
        }
