...
impl MultiSigTreasuryManager {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        tracing::info!("🔐 Multi-signature treasury manager initialized with {} wallets", self.wallets.len());
        Ok(())
    }
    ...
    pub async fn create_treasury_wallet(
        ...
        if threshold == 0 || threshold > signers.len() as u32 {
            return Err(anyhow!("Invalid threshold: must be between 1 and {}", signers.len()));
        }
        ...
        tracing::info!("🏦 Created multi-sig treasury wallet '{}' (ID: {}) with {}/{} threshold", 
                      name, wallet_id, threshold, signers.len());
        Ok(wallet_id)
    }
    ...
    pub async fn propose_transaction(
        ...
        let _proposer = wallet.signers.iter()
        ...
        tracing::info!("📝 Transaction proposed for wallet {} by signer {} (TX: {})", 
                      wallet_id, proposer_id, transaction_id);
        Ok(transaction_id)
    }
    ...
    pub async fn sign_transaction(
        ...
        let pending_tx = self.pending_transactions.get_mut(&transaction_id)
        ...
        let wallet = self.wallets.get(&pending_tx.wallet_id)
        ...
        let signature_weight: u32 = pending_tx.signatures.keys()
        ...
        if is_ready {
            ...
            tracing::info!("✅ Transaction {} ready to execute ({}/{} weight threshold met)", 
                          transaction_id, signature_weight, wallet.threshold);
        }
        ...
        tracing::info!("🖊️ Transaction {} signed by {} (weight: {}, total: {}/{})", 
                      transaction_id, signer.name, signer.weight, signature_weight, wallet.threshold);
    ...
    pub async fn execute_transaction(&mut self, transaction_id: Uuid) -> Result<Transaction> {
        ...
        if signature_weight < wallet.threshold {
            return Err(anyhow!("Insufficient signature weight: {}/{}", signature_weight, wallet.threshold));
        }
        ...
        for (signer_id, tx_sig) in &pending_tx.signatures {
            ...
            let public_key_bytes: [u8; 32] = signer.public_key.clone().try_into().map_err(|_| anyhow!("Invalid public key length"))?;
            let verifying_key = VerifyingKey::from_bytes(&public_key_bytes)?;
            
            let signature_bytes: [u8; 64] = tx_sig.signature.clone().try_into().map_err(|_| anyhow!("Invalid signature length"))?;
            let signature = Signature::from_bytes(&signature_bytes);
            
            verifying_key.verify_strict(&transaction_bytes, &signature)
        ...
        tracing::info!("🚀 Multi-sig transaction {} executed successfully from wallet {}", 
                      transaction_id, wallet.name);
    ...
    pub async fn get_pending_transactions(&self, wallet_id: Option<Uuid>) -> Vec<&PendingTransaction> {
        self.pending_transactions.values()
            .filter(|tx| wallet_id.map_or(true, |id| tx.wallet_id == id))
            .filter(|tx| matches!(tx.status, TransactionStatus::AwaitingSignatures | TransactionStatus::ReadyToExecute))
            .collect()
    }
    ...
    pub async fn cleanup_expired_transactions(&mut self) -> Result<u32> {
        ...
        let expired_ids: Vec<Uuid> = self.pending_transactions.iter()
            .filter(|(_, tx)| current_time > tx.expires_at && matches!(tx.status, TransactionStatus::AwaitingSignatures))
            .map(|(id, _)| *id)
            .collect();
        ...
        if expired_count > 0 {
            tracing::info!("🧹 Cleaned up {} expired multi-sig transactions", expired_count);
        }
    ...
    async fn load_pending_transactions(&mut self) -> Result<()> {
        ...
        for row in rows {
            ...
            let signatures: HashMap<Uuid, TransactionSignature> = serde_json::from_str(&signatures_json)?;
