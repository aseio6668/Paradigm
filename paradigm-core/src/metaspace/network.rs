...
pub enum StorageMessage {
    ...
    ProofChallenge { challenge: StorageChallenge },

    /// Storage proof response
    ProofResponse { proof: StorageProof },

    /// Keeper heartbeat and status
    Heartbeat { keeper: Keeper, sigil_count: usize },

    /// Request keeper status
    StatusRequest { requester_id: String },

    /// Keeper status response
    StatusResponse { keeper: Keeper, online: bool },
...
impl StorageNetworkClient {
    ...
    pub async fn send_proof_challenge(
        &self,
        keeper_address: &str,
    ...
    /// Get keeper status
    pub async fn get_keeper_status(
        &self,
        keeper_address: &str,
        requester_id: &str,
    ) -> Result<(Keeper, bool)> {
...
impl StorageNetworkServer {
    ...
    pub async fn start(&mut self, bind_address: &str) -> Result<()> {
        ...
        println!(
            "🔗 Keeper {} listening on {}",
            self.keeper.keeper_id, bind_address
        );
        ...
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) =
                    Self::handle_connection(stream, &mut keeper_clone, &storage_path_clone).await
                {
    ...
    async fn handle_message(message: StorageMessage, keeper: &mut Keeper) -> StorageMessage {
        match message {
            ...
            StorageMessage::RetrieveRequest {
                sigil_hash,
                requester_id,
                ..
            } => {
            ...
            StorageMessage::ProofChallenge { challenge } => {
                let proof_result = keeper
                    .generate_storage_proof(&challenge.sigil_hash, &challenge.challenge_data)
                    .await;

                match proof_result {
                    Ok(proof) => StorageMessage::ProofResponse { proof },
                    Err(_) => {
...
impl KeeperDiscovery {
    ...
    pub async fn select_keepers_for_storage(
        &mut self,
        count: usize,
