...
impl StorageRewardEngine {
    ...
    pub fn calculate_storage_reward(
        ...
        for sigil in sigils {
            ...
            let base_reward = (size_gb * self.reward_rates.storage_rate_per_gb_month as f64 * days_fraction) as Amount;
            ...
            let reputation_multiplier = 1.0 + (keeper.reputation * (self.reward_rates.reputation_multiplier_max - 1.0));
            
            let adjusted_reward = (base_reward as f64 * importance_multiplier * reputation_multiplier) as Amount;
    ...
    pub fn award_proof_bonus(&mut self, keeper_id: String, proof_result: &ProofVerificationResult, sigil_hash: String) -> Result<Amount> {
        let reward = if proof_result.valid {
            ...
            return self.apply_penalty(keeper_id, self.reward_rates.proof_failure_penalty, sigil_hash);
        };
        ...
        self.reward_history.push(RewardTransaction {
            ...
            metadata: {
                ...
                meta.insert("verification_time_ms".to_string(), proof_result.verification_time_ms.to_string());
                meta
            },
        });
        
        tracing::info!("💰 Awarded {} PAR proof bonus to keeper {} (score: {})", 
                      reward as f64 / 1_00000000.0, keeper_id, proof_result.score);
        
        Ok(reward)
    }
    
    /// Award a retrieval bonus for successful data retrieval
    pub fn award_retrieval_bonus(&mut self, keeper_id: String, response_time_ms: u64, sigil_hash: String) -> Result<Amount> {
        ...
        tracing::info!("💰 Awarded {} PAR retrieval bonus to keeper {} ({}ms)", 
                      reward as f64 / 1_00000000.0, keeper_id, response_time_ms);
    ...
    pub fn award_uptime_bonus(&mut self, keeper_id: String, uptime_hours: f64) -> Result<Amount> {
        ...
        tracing::info!("💰 Awarded {} PAR uptime bonus to keeper {} ({:.1}h uptime)", 
                      reward as f64 / 1_00000000.0, keeper_id, uptime_hours);
        
        Ok(reward)
    }
    
    /// Apply a penalty (negative reward)
    fn apply_penalty(&mut self, keeper_id: String, penalty_amount: Amount, sigil_hash: String) -> Result<Amount> {
        ...
        tracing::warn!("⚠️ Applied {} PAR penalty to keeper {}", 
                      penalty_amount as f64 / 1_00000000.0, keeper_id);
    ...
    pub async fn distribute_rewards(&mut self) -> Result<Vec<RewardDistribution>> {
        ...
        for (keeper_id, amount) in self.pending_rewards.drain() {
            if amount > 0 && amount <= self.reward_pool_balance {
                ...
                tracing::info!("💸 Distributed {} PAR to keeper {}", 
                              amount as f64 / 1_00000000.0, keeper_id);
            } else if amount > self.reward_pool_balance {
                tracing::warn!("💰 Insufficient reward pool balance for keeper {} (need: {}, have: {})", 
                              keeper_id, amount as f64 / 1_00000000.0, self.reward_pool_balance as f64 / 1_00000000.0);
    ...
    pub fn add_to_reward_pool(&mut self, amount: Amount) {
        self.reward_pool_balance += amount;
        tracing::info!("💰 Added {} PAR to storage reward pool (new balance: {} PAR)", 
                      amount as f64 / 1_00000000.0, 
                      self.reward_pool_balance as f64 / 1_00000000.0);
    }
    
    /// Get reward history for a keeper
    pub fn get_keeper_reward_history(&self, keeper_id: &str, limit: Option<usize>) -> Vec<RewardTransaction> {
        let mut history: Vec<_> = self.reward_history
            .iter()
    ...
    pub fn get_reward_statistics(&self) -> RewardStatistics {
        let total_rewards_paid: Amount = self.reward_history
            .iter()
            .filter(|tx| tx.reward_type != RewardType::Penalty)
            .map(|tx| tx.amount)
            .sum();
            
        let total_penalties: Amount = self.reward_history
            .iter()
            .filter(|tx| tx.reward_type == RewardType::Penalty)
            .map(|tx| tx.amount)
            .sum();
...
mod tests {
    use super::*;
    use super::super::glyph::{Element, DataCategory, Importance};
    ...
    fn test_storage_reward_calculation() {
        ...
        // Create test keeper
        let mut keeper = super::super::keeper::Keeper::new("127.0.0.1:8080".to_string(), 1024 * 1024 * 1024); // 1GB capacity
        keeper.reputation = 0.8; // High reputation
        
        // Create test sigil
        let glyph = super::super::glyph::Glyph::new(Element::Earth, DataCategory::Archive, Importance::Normal);
    ...
    fn test_proof_bonus() {
        ...
        let reward = engine.award_proof_bonus(
    ...
    fn test_penalty_system() {
        ...
        // Apply penalty
        let penalty = engine.apply_penalty(
            "test_keeper".to_string(),
            50_00000000,
            "test_sigil".to_string(),
        ).unwrap();
