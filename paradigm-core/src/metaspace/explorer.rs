...
impl SigilExplorer {
    ...
    fn matches_filters(&self, sigil: &Sigil) -> bool {
        ...
        // Element filter
        if !self.filters.elements.is_empty()
            && !self.filters.elements.contains(&sigil.glyph.element)
        {
            return false;
        }
        ...
        if !self.filters.categories.is_empty()
            && !self.filters.categories.contains(&sigil.glyph.category)
        {
            return false;
        }

        // Importance filter
        if !self.filters.importance_levels.is_empty()
            && !self
                .filters
                .importance_levels
                .contains(&sigil.glyph.importance)
        {
            return false;
        }
        ...
        // Originator filter
        if !self.filters.originators.is_empty()
            && !self.filters.originators.contains(&sigil.originator)
        {
            return false;
        }
    ...
    fn find_related_sigils(
        &self,
        sigil: &Sigil,
        all_sigils: &HashMap<String, Sigil>,
    ) -> Vec<String> {
            ...
            SortField::Size => {
                results.sort_by(|a, b| {
                    let cmp = a
                        .sigil_data
                        .size_formatted
                        .cmp(&b.sigil_data.size_formatted);
                    if self.sort.direction == SortDirection::Ascending {
                        cmp
                    } else {
                        cmp.reverse()
            ...
            SortField::KeeperCount => {
                results.sort_by(|a, b| {
                    let cmp = a
                        .availability
                        .keeper_count
                        .cmp(&b.availability.keeper_count);
                    if self.sort.direction == SortDirection::Ascending {
                        cmp
                    } else {
                        cmp.reverse()
            ...
            SortField::RetrievalCount => {
                results.sort_by(|a, b| {
                    let cmp = a
                        .sigil_data
                        .retrieval_count
                        .cmp(&b.sigil_data.retrieval_count);
                    if self.sort.direction == SortDirection::Ascending {
                        cmp
                    } else {
                        cmp.reverse()
    ...
    fn update_pagination(&mut self, total_results: usize) {
        ...
        self.pagination.total_pages =
            (total_results + self.pagination.page_size - 1) / self.pagination.page_size;
