...
pub enum VisualEffect {
    ...
    SigilFlow { direction: FlowDirection, count: u32 },
    
    /// Reward distribution effect
    TokenBurst { amount: u64 },
    
    /// Status change effect
    StatusChange { from: KeeperStatus, to: KeeperStatus },
...
impl KeeperMap {
    ...
    pub async fn update_keepers(&mut self, keepers: &HashMap<String, Keeper>) -> Result<()> {
        ...
        self.keeper_positions.retain(|id, _| keepers.contains_key(id));
    ...
    fn status_color(&self, keeper: &Keeper) -> Color {
        match keeper.status {
            KeeperStatus::Loremaster => Color { r: 255, g: 215, b: 0, a: 255 }, // Gold
            KeeperStatus::Archivist => Color { r: 138, g: 43, b: 226, a: 255 }, // Blue Violet
            KeeperStatus::Guardian => Color { r: 0, g: 128, b: 0, a: 255 }, // Green
            KeeperStatus::Apprentice => Color { r: 70, g: 130, b: 180, a: 255 }, // Steel Blue
            KeeperStatus::Probation => Color { r: 255, g: 140, b: 0, a: 255 }, // Orange
            KeeperStatus::Dormant => Color { r: 128, g: 128, b: 128, a: 255 }, // Gray
        }
    }
    
    fn reputation_color(&self, keeper: &Keeper) -> Color {
        let intensity = (keeper.reputation * 255.0) as u8;
        Color { r: 0, g: intensity, b: 255 - intensity, a: 255 }
    }
    
    fn storage_color(&self, keeper: &Keeper) -> Color {
        let utilization = keeper.storage_utilization() as u8;
        Color { r: utilization, g: 255 - utilization, b: 0, a: 255 }
    }
    ...
    fn earnings_color(&self, keeper: &Keeper) -> Color {
        ...
        Color { r: intensity, g: intensity / 2, b: 255 - intensity, a: 255 }
    }
    
    fn default_color(&self, keeper: &Keeper) -> Color {
        if keeper.is_online() {
            Color { r: 0, g: 255, b: 0, a: 255 } // Green for online
        } else {
            Color { r: 128, g: 128, b: 128, a: 255 } // Gray for offline
        }
    }
    ...
    async fn update_network_topology(&mut self, keepers: &HashMap<String, Keeper>) -> Result<()> {
        ...
        for (keeper_id, keeper) in keepers {
            status_groups.entry(keeper.status.clone())
    ...
    fn calculate_cluster_center(&self, keeper_ids: &[String]) -> Position2D {
        ...
        if count > 0 {
            Position2D { x: total_x / count as f32, y: total_y / count as f32 }
        } else {
            Position2D { x: self.layout.canvas_size.0 / 2.0, y: self.layout.canvas_size.1 / 2.0 }
        }
    }
    
    fn calculate_cluster_health(&self, status: &KeeperStatus, keepers: &HashMap<String, Keeper>) -> f32 {
        let cluster_keepers: Vec<_> = keepers.values()
        ...
        let avg_reputation: f32 = cluster_keepers.iter()
            .map(|k| k.reputation)
            .sum::<f64>() as f32 / cluster_keepers.len() as f32;
        
        let online_ratio = cluster_keepers.iter()
            .filter(|k| k.is_online())
            .count() as f32 / cluster_keepers.len() as f32;
    ...
    async fn update_connections(&mut self, keepers: &HashMap<String, Keeper>) -> Result<()> {
        ...
        for (keeper_id, keeper) in keepers {
            for sigil_hash in &keeper.sigils_held {
                sigil_keepers.entry(sigil_hash.clone())
                    .or_insert_with(Vec::new)
                    .push(keeper_id.clone());
            }
        }
    ...
    fn run_force_directed_layout(&mut self) -> Result<()> {
        ...
        let positions: Vec<_> = self.keeper_positions.iter()
        ...
        for (keeper_id, (force_x, force_y)) in force_updates {
            if let Some(keeper_pos) = self.keeper_positions.get_mut(&keeper_id) {
                ...
                // Keep within bounds
                keeper_pos.position.x = keeper_pos.position.x.clamp(50.0, self.layout.canvas_size.0 - 50.0);
                keeper_pos.position.y = keeper_pos.position.y.clamp(50.0, self.layout.canvas_size.1 - 50.0);
            }
        }
    ...
    fn run_hierarchical_layout(&mut self) -> Result<()> {
        ...
        let mut keepers_with_reputation: Vec<_> = keeper_ids.iter()
            .map(|id| (id.clone(), self.get_keeper_reputation(id)))
            .collect();
        
        // Sort by reputation (descending)
        keepers_with_reputation.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
