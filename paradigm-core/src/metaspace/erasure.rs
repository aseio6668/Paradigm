...
impl ErasureEncoder {
    ...
    pub fn new(data_shards: usize, parity_shards: usize) -> Result<Self> {
        if data_shards == 0 || parity_shards == 0 {
            return Err(anyhow!("Must have at least 1 data shard and 1 parity shard"));
        }
    ...
    pub fn decode(&self, shards: Vec<DataShard>) -> Result<Vec<u8>> {
        if shards.len() < self.data_shards {
            return Err(anyhow!("Not enough shards to reconstruct data. Need at least {} data shards", self.data_shards));
        }
        ...
        for shard in &shards {
            ...
            if calculated_checksum != shard.checksum {
                return Err(anyhow!("Shard {} failed checksum verification", shard.index));
            }
        }
        ...
        if data_shards.len() < self.data_shards {
            ...
            return Err(anyhow!("Missing data shards reconstruction not yet implemented"));
        }
...
pub fn create_shards(data: Vec<u8>, data_shards: usize, parity_shards: usize) -> Result<Vec<DataShard>> {
...
mod tests {
    ...
    fn test_erasure_coding_basic() {
        ...
        let data_only_shards = shards.into_iter()
            .filter(|s| s.is_data_shard)
            .collect();
