...
impl ProofEngine {
    ...
    pub fn issue_storage_challenge(&mut self, keeper_id: &str, sigil_hash: &str) -> StorageChallenge {
        let challenge_id = format!("challenge_{}_{}", 
                                   keeper_id, 
    ...
    pub fn verify_storage_proof(&mut self, proof: &StorageProof) -> ProofVerificationResult {
        ...
        ProofVerificationResult {
            valid,
            verification_time_ms: start_time.elapsed().as_millis() as u64,
            error_message: if valid { None } else { Some("Proof verification failed".to_string()) },
            score,
        }
    }
    
    /// Generate the expected proof for a challenge
    fn generate_expected_proof(&self, sigil_hash: &str, challenge_data: &[u8], keeper_id: &str) -> Vec<u8> {
        let mut hasher = Sha256::new();
    ...
    pub fn issue_retrievability_challenge(&mut self, keeper_id: &str, sigil_hash: &str, data_size: usize) -> Vec<(usize, usize)> {
        ...
        for i in 0..num_ranges {
            if data_size > 100 {
                ...
                let hash_seed = sigil_hash.chars().fold(0u32, |acc, c| acc.wrapping_mul(31).wrapping_add(c as u32));
    ...
    pub fn verify_retrievability_proof(&self, proof: &RetrievabilityProof, expected_data: &[u8]) -> ProofVerificationResult {
        let start_time = std::time::Instant::now();
        let mut score = 0.0;
        ...
        for (range_start, range_end) in &proof.requested_ranges {
            if *range_end > expected_data.len() {
                errors.push(format!("Range {}..{} exceeds data size", range_start, range_end));
                continue;
            }
            ...
            if proof_range_end > proof.retrieved_data.len() {
                errors.push(format!("Insufficient proof data for range {}..{}", range_start, range_end));
                continue;
            }
            ...
            if expected_range == proof_range {
                ...
                errors.push(format!("Data mismatch in range {}..{}", range_start, range_end));
            }
        ...
        if proof.response_time_ms > 5000 { // > 5 seconds
            score *= 0.5;
        } else if proof.response_time_ms > 1000 { // > 1 second
            score *= 0.8;
        }
...
mod tests {
    ...
    fn test_retrievability_proof() {
        ...
        let ranges = engine.issue_retrievability_challenge("keeper_1", "sigil_123", test_data.len());
    ...
    fn test_expired_challenge() {
        ...
        engine.active_challenges.insert("test_challenge".to_string(), expired_challenge);
