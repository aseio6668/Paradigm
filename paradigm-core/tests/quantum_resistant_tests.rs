...
async fn test_quantum_resistant_crypto_initialization() {
    ...
    assert!(
        result.is_ok(),
        "Quantum-resistant crypto should initialize successfully"
    );
}
...
async fn test_lattice_based_signatures() {
    ...
    crypto
        .generate_contributor_keys(&address)
        .await
    ...
    let signature_result = crypto
        .sign_contribution_proof(&address, test_data, QRSignatureType::Lattice)
        .await;

    assert!(
        signature_result.is_ok(),
        "Lattice signature should be created"
    );

    let signature = signature_result.unwrap();
    assert_eq!(signature.signature_type, QRSignatureType::Lattice);
    assert!(
        !signature.signature_data.is_empty(),
        "Signature data should be present"
    );

    // Test signature verification
    let verification_result = crypto
        .verify_signature(&address, test_data, &signature)
        .await;
    assert!(
    ...
    let wrong_verification = crypto
        .verify_signature(&address, wrong_data, &signature)
        .await;
...
async fn test_hash_based_signatures() {
    ...
    let signature_result = crypto
        .sign_contribution_proof(&address, test_data, QRSignatureType::HashBased)
        .await;

    assert!(
        signature_result.is_ok(),
        "Hash-based signature should be created"
    );
...
async fn test_invalid_zk_proof_type() {
    ...
    let result = crypto
        .generate_qr_zk_proof("invalid_proof_type", &private_inputs, &public_inputs)
        .await;
...
async fn test_post_quantum_key_exchange() {
    ...
    assert!(
        exchange1_result.is_ok(),
        "Key exchange with peer1 should succeed"
    );

    let secret1 = exchange1_result.unwrap();
    assert!(
        !secret1.secret.is_empty(),
    ...
    assert!(
        exchange2_result.is_ok(),
        "Key exchange with peer2 should succeed"
    );

    let secret2 = exchange2_result.unwrap();
    assert!(
        !secret2.secret.is_empty(),
        "Shared secret should be generated"
    );
    ...
    assert_ne!(
        ...
        "Secrets with different peers should differ"
    );
}
...
async fn test_quantum_random_oracle() {
    ...
    assert!(
        random_result.is_ok(),
        "Quantum random generation should succeed"
    );

    let random1 = random_result.unwrap();
    assert!(
        !random1.value.is_empty(),
        "Random value should be generated"
    );
    ...
    assert!(
        random2_result.is_ok(),
        "Second random generation should succeed"
    );

    let random2 = random2_result.unwrap();
    assert_ne!(
        random1.value, random2.value,
        "Different entropy should produce different random values"
    );
...
async fn test_signature_type_consistency() {
    ...
    let lattice_sig = crypto
        .sign_contribution_proof(&address, test_data, QRSignatureType::Lattice)
        .await
        .expect("Lattice signature should be created");

    // Create hash-based signature
    let hash_sig = crypto
        .sign_contribution_proof(&address, test_data, QRSignatureType::HashBased)
        .await
        .expect("Hash-based signature should be created");
    ...
    assert!(crypto
        .verify_signature(&address, test_data, &lattice_sig)
        .await
        .unwrap());
    assert!(crypto
        .verify_signature(&address, test_data, &hash_sig)
        .await
        .unwrap());
}
...
async fn test_key_exchange_uniqueness() {
    ...
    for i in 0..3 {
        let secret = crypto
            .quantum_safe_key_exchange(&peer)
            .await
...
async fn test_comprehensive_cryptographic_workflow() {
    ...
    let alice_keys = crypto
        .generate_contributor_keys(&alice)
        .await
        .expect("Alice's keys should be generated");
    let bob_keys = crypto
        .generate_contributor_keys(&bob)
        .await
    ...
    let alice_proof = crypto
        .generate_qr_zk_proof("ml_training", &private_inputs, &public_inputs)
        .await
    ...
    let alice_secret = crypto
        .quantum_safe_key_exchange(&bob)
        .await
        .expect("Alice should establish shared secret with Bob");
    let bob_secret = crypto
        .quantum_safe_key_exchange(&alice)
        .await
        .expect("Bob should establish shared secret with Alice");

    // In a real implementation, both parties would derive the same secret
    assert!(
        !alice_secret.secret.is_empty(),
        "Alice's shared secret should exist"
    );
    assert!(
        !bob_secret.secret.is_empty(),
        "Bob's shared secret should exist"
    );

    // 6. Generate quantum randomness for governance
    let governance_random = crypto
        .get_quantum_random(vec![
            "governance_proposal_123".to_string(),
            format!("alice_address_{:?}", alice.0[0]),
            format!("bob_address_{:?}", bob.0[0]),
        ])
        .await
        .expect("Quantum randomness should be generated");

    assert!(
        !governance_random.value.is_empty(),
        "Governance randomness should be generated"
    );
    assert_eq!(
        governance_random.entropy_sources.len(),
        3,
        "Should use all entropy sources"
    );
}
...
async fn test_error_handling() {
    ...
    crypto
        .generate_contributor_keys(&valid_address)
        .await
        .expect("Keys should be generated");
    ...
    let result = crypto
        .sign_contribution_proof(&invalid_address, test_data, QRSignatureType::Lattice)
        .await;
    assert!(
        result.is_err(),
        "Should fail to sign with non-existent keys"
    );

    // Try to verify with address that has no keys
    let valid_signature = crypto
        .sign_contribution_proof(&valid_address, test_data, QRSignatureType::Lattice)
        .await
        .expect("Valid signature should be created");

    let verify_result = crypto
        .verify_signature(&invalid_address, test_data, &valid_signature)
        .await;
    assert!(
        verify_result.is_err(),
        "Should fail to verify with non-existent keys"
    );
}
...
async fn test_performance_characteristics() {
    ...
    assert!(
        sig_time.as_millis() / 10 < 100,
        "Average signature should complete within 100ms"
    );
}
