...
async fn test_advanced_governance_initialization() {
    ...
    assert!(result.is_ok(), "Advanced governance should initialize successfully");
}

#[tokio::test]
async fn test_quadratic_proposal_creation() {
    let mut governance = AdvancedGovernance::new();
    governance.initialize().await.expect("Governance should initialize");
    ...
    let result = governance.create_quadratic_proposal(
        proposer,
        proposal_data.clone(),
        quantum_random,
    ).await;
...
async fn test_quadratic_voting_mechanics() {
    ...
    let proposal_id = governance.create_quadratic_proposal(
        proposer,
        proposal_data,
        create_test_quantum_random(),
    ).await.expect("Proposal should be created");
    ...
    for (i, (&vote_strength, &max_cost)) in vote_strengths.iter().zip(max_costs.iter()).enumerate() {
        ...
        let vote_result = governance.cast_quadratic_vote(
            test_voter,
            proposal_id,
            vote_strength,
            max_cost,
        ...
        if vote_strength >= 0.0 {
            assert!(vote.effective_votes >= 0.0, "Positive vote should have positive effect");
        } else {
            assert!(vote.effective_votes < 0.0, "Negative vote should have negative effect");
        }
    }
}
...
async fn test_vote_cost_exceeds_maximum() {
    ...
    let result = governance.cast_quadratic_vote(
        voter,
        proposal_id,
    ...
    assert!(result.is_err(), "Vote should fail when cost exceeds maximum");
}
...
async fn test_futarchy_market_creation_and_betting() {
    ...
    let market_result = governance.create_futarchy_market(
        proposer,
        proposal_data,
        success_metrics,
    ).await;
    ...
    let bet_result = governance.place_futarchy_bet(
        bettor,
        market_id,
...
async fn test_futarchy_market_resolution() {
    ...
    let market_id = governance.create_futarchy_market(
        proposer,
        proposal_data,
        success_metrics,
    ).await.expect("Market should be created");
    ...
    let resolution_result = governance.resolve_futarchy_market(market_id, actual_outcomes).await;
    
    assert!(resolution_result.is_ok(), "Market resolution should succeed");
    
    let resolution = resolution_result.unwrap();
    assert_eq!(resolution.market_id, market_id);
    assert!(resolution.implement_accuracy >= 0.0, "Accuracy should be non-negative");
    assert!(resolution.no_implement_accuracy >= 0.0, "Accuracy should be non-negative");
}
...
async fn test_conviction_voting_system() {
    ...
    let proposal_result = governance.start_conviction_voting(
        proposer,
        proposal_data,
        50000, // 50k tokens requested
    ).await;
    
    assert!(proposal_result.is_ok(), "Conviction proposal creation should succeed");
    let proposal_id = proposal_result.unwrap();
    
    // Test conviction signaling
    let signal1_result = governance.signal_conviction(
        supporter1,
        proposal_id,
        5000, // 5k tokens
    ).await;
    ...
    assert!(!signal1.funded, "Should not be funded yet with small signal");
    
    // Add more conviction
    let signal2_result = governance.signal_conviction(
        supporter2,
        proposal_id,
        15000, // 15k tokens
    ).await;
    ...
    if signal2.total_conviction >= signal2.funding_threshold {
        assert!(signal2.funded, "Proposal should be funded when threshold is reached");
    }
}
...
async fn test_ai_agent_proposal_assessment() {
    ...
    let assessment_result = governance.ai_agent_system.assess_proposal(&proposal_data).await;
    ...
    assert!(!assessment.individual_assessments.is_empty(), "Should have individual assessments");
    ...
    for agent_assessment in &assessment.individual_assessments {
        assert!(agent_assessment.score >= 0.0 && agent_assessment.score <= 1.0,
               "Agent score should be between 0 and 1");
        assert!(!agent_assessment.agent_name.is_empty(), "Agent should have a name");
        assert!(!agent_assessment.reasoning.is_empty(), "Agent should provide reasoning");
    }
}
...
async fn test_ai_agent_learning_from_votes() {
    ...
    let learning_result = governance.ai_agent_system.update_assessment_from_vote(
        proposal_id,
        &vote,
...
async fn test_delegation_system() {
    ...
    let full_delegation_result = governance.delegate_voting_power(
        delegator.clone(),
        delegatee.clone(),
        DelegationType::Full,
    ...
    assert!(full_delegation_result.is_ok(), "Full delegation should succeed");
    
    let delegation = full_delegation_result.unwrap();
    assert!(delegation.active, "Delegation should be active");
    
    // Test specific delegation
    let specific_delegation_result = governance.delegate_voting_power(
        delegator.clone(),
        delegatee.clone(),
        DelegationType::Specific(vec![ProposalType::TreasuryAllocation, ProposalType::ParameterChange]),
    ...
    let temp_delegation_result = governance.delegate_voting_power(
        delegator,
        delegatee,
    ...
    assert!(temp_delegation_result.is_ok(), "Temporary delegation should succeed");
}
...
async fn test_governance_statistics() {
    ...
    let _proposal1 = governance.create_quadratic_proposal(
        proposer.clone(),
        ...
        create_test_quantum_random(),
    ).await.expect("First proposal should be created");
    
    let _proposal2 = governance.create_quadratic_proposal(
        proposer,
        QuadraticProposalData {
        ...
        create_test_quantum_random(),
    ).await.expect("Second proposal should be created");
    ...
    let stats = stats_result.unwrap();
    assert_eq!(stats.active_quadratic_proposals, 2, "Should have 2 active quadratic proposals");
    assert_eq!(stats.total_proposals_created, 2, "Should track total proposals created");
    assert!(stats.governance_participation_rate >= 0.0, "Participation rate should be non-negative");
    assert!(stats.governance_participation_rate <= 1.0, "Participation rate should not exceed 100%");
    
    // Check subsystem stats
    assert!(stats.ai_agent_stats.total_agents > 0, "Should have AI agents");
    assert!(stats.conviction_stats.active_proposals >= 0, "Should track conviction proposals");
    assert!(stats.delegation_stats.total_delegations >= 0, "Should track delegations");
}
...
async fn test_proposal_type_cost_differences() {
    ...
    for (i, &proposal_id) in proposal_ids.iter().enumerate() {
        ...
        let vote_result = governance.cast_quadratic_vote(
            ...
            vote_strength,
            max_cost,
        ).await.expect("Vote should succeed");
    ...
    assert!(emergency_cost >= parameter_cost, 
...
async fn test_comprehensive_governance_workflow() {
    ...
    let ai_assessment = governance.ai_agent_system.assess_proposal(&proposal_data).await
        .expect("AI assessment should work");
    assert!(ai_assessment.overall_score > 0.7, "Should be highly rated by AI");
    
    // 4. Cast votes
    governance.cast_quadratic_vote(voter1.clone(), proposal_id, 6.0, 15000).await
        .expect("First vote should succeed");
    
    governance.cast_quadratic_vote(voter2.clone(), proposal_id, -2.0, 5000).await
    ...
    let market_id = governance.create_futarchy_market(
        ...
        success_metrics,
    ).await.expect("Futarchy market should be created");
    
    // 6. Place bets in market
    governance.place_futarchy_bet(
        voter1,
        market_id,
        FutarchyMarketType::Implement,
        OutcomeBet {
        ...
        2000,
    ).await.expect("Futarchy bet should succeed");
    
    // 7. Start conviction voting for implementation funding
    let funding_proposal_id = governance.start_conviction_voting(
        proposer,
        ...
        1000000, // 1M tokens
    ).await.expect("Conviction voting should start");
    
    // 8. Signal conviction
    governance.signal_conviction(voter1, funding_proposal_id, 25000).await
        .expect("Conviction signaling should succeed");
    
    governance.signal_conviction(voter2, funding_proposal_id, 15000).await
        .expect("Second conviction signal should succeed");
    
    // 9. Get final statistics
    let final_stats = governance.get_governance_stats().await
        .expect("Final stats should be available");
    
    assert!(final_stats.active_quadratic_proposals > 0, "Should have active proposals");
    assert!(final_stats.active_futarchy_markets > 0, "Should have active markets");
    assert!(final_stats.conviction_stats.active_proposals > 0, "Should have conviction proposals");
    assert!(final_stats.total_proposals_created >= 2, "Should track all created proposals");
}
...
async fn test_error_conditions() {
    ...
    assert!(vote_result.is_err(), "Should fail to vote on non-existent proposal");
    
    // Test conviction signaling on non-existent proposal
    let conviction_result = governance.signal_conviction(
        voter,
        invalid_proposal_id,
        5000,
    ).await;
    
    assert!(conviction_result.is_err(), "Should fail to signal conviction on non-existent proposal");
}
...
async fn test_governance_performance() {
    ...
    assert!(init_time.as_millis() < 1000, "Governance should initialize quickly");
    ...
    for i in 0..5 {
        ...
        governance.create_quadratic_proposal(
            proposer.clone(),
            proposal_data,
            create_test_quantum_random(),
        ).await.expect("Proposal should be created");
    ...
    assert!(proposal_time.as_millis() / 5 < 100, "Average proposal creation should be fast");
}