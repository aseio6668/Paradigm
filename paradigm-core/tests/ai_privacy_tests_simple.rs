...
async fn test_basic_tokenomics_optimization() {
    ...
    assert!(optimized_params.base_reward_multiplier >= 0.0, "Base reward multiplier should be non-negative");
}
...
async fn test_ai_privacy_performance() {
    ...
    assert!(ai_init_time.as_millis() < 5000, "AI init should be under 5 seconds");
    assert!(privacy_init_time.as_millis() < 5000, "Privacy init should be under 5 seconds");
}
