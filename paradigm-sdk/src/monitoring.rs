...
impl MonitoringSystem {
    ...
    pub async fn record_transaction(&self, transaction: &Transaction, duration: Duration, success: bool) {
        // Update metrics
        self.metrics_collector.increment_counter("transactions_total").await;
        if success {
            self.metrics_collector.increment_counter("transactions_success").await;
        } else {
            self.metrics_collector.increment_counter("transactions_error").await;
        }
        
        // Record timing
        self.metrics_collector.record_timer("transaction_duration", duration).await;
        
        // Update performance tracker
        self.performance_tracker.record_operation("transaction", duration, success).await;
        
        // Record transaction amount histogram
        let amount_eth = transaction.amount.to_paradigm();
        self.metrics_collector.record_histogram("transaction_amount", amount_eth).await;
    }
    
    /// Record a signature operation
    pub async fn record_signature_operation(&self, operation: &str, duration: Duration, success: bool) {
        let metric_name = format!("signature_{}", operation);
        
        self.metrics_collector.increment_counter(&format!("{}_total", metric_name)).await;
        if success {
            self.metrics_collector.increment_counter(&format!("{}_success", metric_name)).await;
        } else {
            self.metrics_collector.increment_counter(&format!("{}_error", metric_name)).await;
        }
        
        self.metrics_collector.record_timer(&format!("{}_duration", metric_name), duration).await;
        self.performance_tracker.record_operation(&metric_name, duration, success).await;
    }
...
impl MetricsCollector {
    ...
    pub async fn record_timer(&self, name: &str, duration: Duration) {
        ...
        let timer = timers.entry(name.to_string()).or_insert_with(|| Timer::new(1000));
    ...
    pub async fn collect_system_metrics(&self) {
        ...
        self.set_gauge("system_cpu_usage", resource_usage.cpu_usage).await;
        self.set_gauge("system_memory_usage", resource_usage.memory_usage).await;
        self.set_gauge("system_disk_usage", resource_usage.disk_usage).await;
        self.set_gauge("system_network_rx", resource_usage.network_rx as f64).await;
        self.set_gauge("system_network_tx", resource_usage.network_tx as f64).await;
        self.set_gauge("system_open_connections", resource_usage.open_connections as f64).await;
        self.set_gauge("system_thread_count", resource_usage.thread_count as f64).await;
    }
...
impl HealthMonitor {
    ...
    async fn update_health_history(&self) {
        ...
        let snapshot = HealthSnapshot {
            ...
            component_statuses: components.iter()
                .map(|(name, health)| (name.clone(), health.status.clone()))
                .collect(),
            active_issues: components.iter()
...
impl PerformanceTracker {
    ...
    pub async fn record_operation(&self, operation: &str, duration: Duration, success: bool) {
        ...
        let operation_metrics = metrics.entry(operation.to_string()).or_insert_with(|| {
            OperationMetrics::new(operation.to_string())
        });
    ...
    async fn update_baselines(&self) {
        ...
        for (operation, operation_metrics) in metrics.iter() {
            if operation_metrics.total_count >= 100 { // Enough samples for baseline
...
impl OperationMetrics {
    ...
    pub fn record(&mut self, duration: Duration, success: bool) {
        ...
        self.avg_duration = Duration::from_nanos(
            (self.avg_duration.as_nanos() as u64 * (self.total_count - 1) + duration.as_nanos() as u64) / self.total_count
        );
        
        // Mock percentile calculations
        self.p50_duration = self.avg_duration;
        self.p95_duration = Duration::from_nanos((self.avg_duration.as_nanos() as f64 * 1.5) as u64);
        self.p99_duration = Duration::from_nanos((self.avg_duration.as_nanos() as f64 * 2.0) as u64);
        ...
        for (operation, operation_metrics) in metrics {
            ...
            if operation_metrics.avg_duration > Duration::from_millis(1000) {
                anomalies.push(PerformanceAnomaly {
                    ...
                    description: format!("High latency detected: {:?}", operation_metrics.avg_duration),
                    metrics: [
                        ("avg_duration_ms".to_string(), operation_metrics.avg_duration.as_millis() as f64),
                        ("error_rate".to_string(), operation_metrics.error_count as f64 / operation_metrics.total_count as f64),
                    ].into(),
                });
            }
            ...
            if operation_metrics.throughput < 1.0 && operation_metrics.total_count > 10 {
                anomalies.push(PerformanceAnomaly {
                    ...
                    detected_at: SystemTime::now(),
                    severity: AnomalySeverity::Medium,
                    description: format!("Low throughput detected: {:.2} ops/sec", operation_metrics.throughput),
                    metrics: [
                        ("throughput".to_string(), operation_metrics.throughput),
                        ("total_count".to_string(), operation_metrics.total_count as f64),
                    ].into(),
                });
            }
            
            // Check for high error rate
            let error_rate = operation_metrics.error_count as f64 / operation_metrics.total_count as f64;
            if error_rate > self.detection_thresholds.error_rate_spike {
                anomalies.push(PerformanceAnomaly {
                    ...
                    metrics: [
                        ...
                        ("error_count".to_string(), operation_metrics.error_count as f64),
                        ("total_count".to_string(), operation_metrics.total_count as f64),
                    ].into(),
                });
            }
...
impl AlertingSystem {
    ...
    pub async fn evaluate_alert_rules(&self, metrics: &MetricsCollector) {
        ...
        for rule in rules.iter() {
            ...
            if let Some(current_value) = self.get_metric_value(&all_metrics, &rule.metric) {
                let triggered = self.evaluate_condition(&rule.condition, current_value, rule.threshold);
    ...
    async fn trigger_alert(&self, rule: &AlertRule, current_value: f64) {
        let alert_id = format!("alert_{}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis());
        ...
        let alert = Alert {
            ...
            description: format!("{}: {} {} {}", rule.name, rule.metric, current_value, rule.threshold),
            acknowledged: false,
            resolved_at: None,
        };
...
mod tests {
    ...
    async fn test_metrics_collection() {
        ...
        // Test timer
        collector.record_timer("test_timer", Duration::from_millis(100)).await;
    ...
    async fn test_health_monitoring() {
        ...
        monitor.add_component("database".to_string(), HealthStatus::Healthy).await;
        monitor.add_component("cache".to_string(), HealthStatus::Degraded).await;
    ...
    async fn test_performance_tracking() {
        ...
        tracker.record_operation("test_op", Duration::from_millis(50), true).await;
        tracker.record_operation("test_op", Duration::from_millis(75), true).await;
        tracker.record_operation("test_op", Duration::from_millis(100), false).await;
    ...
    async fn test_monitoring_system_integration() {
        ...
        monitoring.record_transaction(&transaction, Duration::from_millis(100), true).await;
        
        // Test signature operation recording
        monitoring.record_signature_operation("sign", Duration::from_millis(50), true).await;
        
        // Test metrics export
        let json_export = monitoring.export_metrics(ExportFormat::Json).await.unwrap();
        assert!(!json_export.is_empty());
        
        let prometheus_export = monitoring.export_metrics(ExportFormat::Prometheus).await.unwrap();
