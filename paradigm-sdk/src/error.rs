...
impl<T> ErrorExt<T> for Result<T> {
    fn with_context(self, context: ErrorContext) -> Result<T> {
        self.map_err(|e| {
            let details = context.details.iter()
                .map(|(k, v)| format!("{}={}", k, v))
                .collect::<Vec<_>>()
                .join(", ");
...
impl ParadigmError {
    ...
    pub fn recovery_strategy(&self) -> ErrorRecovery {
        match self {
            ...
            ParadigmError::InsufficientFunds { .. } => {
                ErrorRecovery {
    ...
    pub fn category(&self) -> ErrorCategory {
        match self {
            ...
            ParadigmError::Wallet(_) | ParadigmError::Keystore(_) => {
                ErrorCategory::Wallet
            }
            
            ParadigmError::Contract(_) | ParadigmError::Abi(_) => {
                ErrorCategory::SmartContract
            }
...
impl ErrorMetrics {
    ...
    pub fn record_error(&mut self, error: &ParadigmError) {
        ...
        *self.error_counts.entry(category).or_insert(0) += 1;
        ...
        if let Some((_, count)) = self.common_errors.iter_mut().find(|(err, _)| err == &error_string) {
            *count += 1;
        } else {
