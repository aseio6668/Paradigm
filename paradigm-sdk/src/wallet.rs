...
pub use ed25519_dalek::{SigningKey as SecretKey, VerifyingKey as PublicKey};
use rand::rngs::OsRng;
...
impl Wallet {
    ...
    pub async fn initialize(&mut self, password: &str) -> Result<()> {
        if *self.state.read().await != WalletState::Uninitialized {
            return Err(ParadigmError::Wallet(
    ...
    pub async fn unlock(&self, password: &str) -> Result<()> {
        ...
        if let Some(stored_hash) = &self.password_hash {
            ...
            if password_hash != *stored_hash {
                return Err(ParadigmError::Authentication(
    ...
    pub async fn get_statistics(&self) -> Result<WalletStatistics> {
        ...
        let most_active = accounts
            .values()
            .max_by_key(|a| a.transaction_count)
            .map(|a| a.id);

        let newest_account = accounts.values().max_by_key(|a| a.created_at).map(|a| a.id);
...
impl TransactionBuilder {
    ...
    pub async fn auto_nonce(mut self, from: &Address) -> Result<Self> {
        if let Some(client) = &self.client {
            let nonce = client
                .get_nonce(from, crate::client::BlockNumber::Latest)
                .await?;
            self.transaction.nonce = nonce;
        }
        Ok(self)
    ...
    pub async fn build_and_sign(
        ...
        wallet
            .sign_transaction(account_id, &mut transaction)
            .await?;
...
mod tests {
    ...
    async fn test_account_management() {
        ...
        let account_id = wallet
            .create_account("Account 1".to_string())
            .await
            .unwrap();
