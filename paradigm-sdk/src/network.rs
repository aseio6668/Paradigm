    ...
    pub fn mainnet() -> Self {
        Self {
            ...
            slash_conditions: vec![
                ...
                SlashCondition::Downtime { penalty_percent: 1, max_downtime_blocks: 8192 },
            ],
    ...
    pub fn testnet() -> Self {
        Self {
            ...
            slash_conditions: vec![
                ...
                SlashCondition::Downtime { penalty_percent: 0, max_downtime_blocks: 1024 },
            ],
...
pub enum SlashCondition {
    DoubleSign { penalty_percent: u32 },
    Downtime { penalty_percent: u32, max_downtime_blocks: u64 },
    InvalidBlock { penalty_percent: u32 },
    Censorship { penalty_percent: u32 },
}
...
impl NetworkManager {
    ...
    pub async fn ban_peer(&self, peer_id: &str, reason: String) -> Result<()> {
        ...
        if let Some(peer) = peers.get_mut(peer_id) {
            ...
            Err(ParadigmError::NotFound(format!("Peer {} not found", peer_id)))
        }
    }
...
pub mod utils {
    ...
    pub fn validate_config(config: &NetworkConfig) -> Result<()> {
        if config.rpc_endpoints.is_empty() {
            return Err(ParadigmError::Config("No RPC endpoints configured".to_string()));
        }
        ...
        if config.p2p_config.max_inbound_peers == 0 && config.p2p_config.max_outbound_peers == 0 {
            return Err(ParadigmError::Config("No peer connections allowed".to_string()));
        }
    ...
    pub fn calculate_network_score(metrics: &NetworkHealthMetrics) -> u32 {
        ...
        let propagation_score = 100_u32.saturating_sub((metrics.block_propagation_time_ms / 50) as u32);
...
mod tests {
    ...
    fn test_consensus_config() {
        ...
        assert_eq!(mainnet_consensus.algorithm, ConsensusAlgorithm::ProofOfStake);
        assert_eq!(mainnet_consensus.block_time_seconds, 12);
        
        let local_consensus = ConsensusConfig::local();
        assert_eq!(local_consensus.algorithm, ConsensusAlgorithm::ProofOfAuthority);
