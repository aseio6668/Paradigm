...
impl ParadigmClient {
    ...
    pub fn with_config(config: ClientConfig) -> Result<Self> {
        ...
        for (key, value) in &config.custom_headers {
            headers.insert(
                reqwest::header::HeaderName::from_bytes(key.as_bytes())
                    .map_err(|e| ParadigmError::Config(format!("Invalid header name '{}': {}", key, e)))?,
                reqwest::header::HeaderValue::from_str(value)
                    .map_err(|e| ParadigmError::Config(format!("Invalid header value '{}': {}", value, e)))?,
            );
        }
        ...
        let endpoints = config.rpc_endpoints.iter().map(|url| RpcEndpoint {
            url: url.clone(),
    ...
    pub async fn get_best_endpoint(&self) -> Result<String> {
        ...
        if connected_endpoints.is_empty() {
            return Err(ParadigmError::Network("No connected endpoints available".to_string()));
        }
    ...
    async fn make_request(&self, url: &str, method: &str, params: &[serde_json::Value]) -> Result<serde_json::Value> {
        ...
        if !response.status().is_success() {
            return Err(ParadigmError::Rpc(format!("HTTP error: {}", response.status())));
        }
    ...
    async fn make_cached_request(&self, method: &str, params: &[serde_json::Value]) -> Result<serde_json::Value> {
    ...
    pub async fn get_block(&self, block_number: BlockNumber) -> Result<Option<Block>> {
        ...
        let result = self.make_cached_request("eth_getBlockByNumber", &[block_param, serde_json::Value::Bool(true)]).await?;
        ...
        let block: Block = serde_json::from_value(result)
            .map_err(|e| ParadigmError::Serialization(format!("Failed to deserialize block: {}", e)))?;
    ...
    pub async fn get_block_by_hash(&self, hash: &Hash) -> Result<Option<Block>> {
        let result = self.make_cached_request("eth_getBlockByHash", &[
    ...
    pub async fn get_transaction(&self, hash: &Hash) -> Result<Option<Transaction>> {
        let result = self.make_cached_request("eth_getTransactionByHash", &[
            serde_json::Value::String(hash.to_string())
        ]).await?;
        ...
        let transaction: Transaction = serde_json::from_value(result)
            .map_err(|e| ParadigmError::Serialization(format!("Failed to deserialize transaction: {}", e)))?;
    ...
    pub async fn get_transaction_receipt(&self, hash: &Hash) -> Result<Option<TransactionReceipt>> {
        let result = self.make_cached_request("eth_getTransactionReceipt", &[
            serde_json::Value::String(hash.to_string())
        ]).await?;
        ...
        let receipt: TransactionReceipt = serde_json::from_value(result)
            .map_err(|e| ParadigmError::Serialization(format!("Failed to deserialize receipt: {}", e)))?;
    ...
    pub async fn get_balance(&self, address: &Address, block: BlockNumber) -> Result<Amount> {
        ...
        let result = self.make_cached_request("eth_getBalance", &[
    ...
    pub async fn get_nonce(&self, address: &Address, block: BlockNumber) -> Result<u64> {
        ...
        let result = self.make_cached_request("eth_getTransactionCount", &[
            serde_json::Value::String(address.to_string()),
            block_param
        ]).await?;
    ...
    pub async fn estimate_gas(&self, transaction: &Transaction) -> Result<u64> {
        ...
        let gas_str = result.as_str()
            .ok_or_else(|| ParadigmError::GasEstimation("Invalid gas estimate format".to_string()))?;
        
        u64::from_str_radix(gas_str.trim_start_matches("0x"), 16)
            .map_err(|e| ParadigmError::GasEstimation(format!("Failed to parse gas estimate: {}", e)))
    }
    ...
    pub async fn wait_for_confirmation(&self, hash: &Hash, confirmations: u64) -> Result<TransactionReceipt> {
        let mut attempts = 0;
