...
use crate::types::*;
use crate::error::{Result, ParadigmError};
...
pub mod format {
    ...
    pub fn format_amount(amount: &Amount) -> String {
        ...
        if paradigm_value.fract() == 0.0 {
            ...
            format!("{:.6} PARADIGM", paradigm_value).trim_end_matches('0').trim_end_matches('.').to_string() + " PARADIGM"
        }
    }
...
pub mod validate {
    ...
    pub fn address(address: &str) -> Result<()> {
        if !address.starts_with("0x") {
            return Err(ParadigmError::InvalidAddress("Address must start with 0x".to_string()));
        }
        
        if address.len() != 42 {
            return Err(ParadigmError::InvalidAddress("Address must be 42 characters long".to_string()));
        }
        
        if !address[2..].chars().all(|c| c.is_ascii_hexdigit()) {
            return Err(ParadigmError::InvalidAddress("Address contains invalid characters".to_string()));
        }
    ...
    pub fn hash(hash: &str) -> Result<()> {
        if !hash.starts_with("0x") {
            return Err(ParadigmError::InvalidHash("Hash must start with 0x".to_string()));
        }
        
        if hash.len() != 66 {
            return Err(ParadigmError::InvalidHash("Hash must be 66 characters long".to_string()));
        }
        
        if !hash[2..].chars().all(|c| c.is_ascii_hexdigit()) {
            return Err(ParadigmError::InvalidHash("Hash contains invalid characters".to_string()));
        }
    ...
    pub fn private_key(key: &str) -> Result<()> {
        let key = if key.starts_with("0x") { &key[2..] } else { key };
        
        if key.len() != 64 {
            return Err(ParadigmError::InvalidKey("Private key must be 32 bytes (64 hex characters)".to_string()));
        }
        
        if !key.chars().all(|c| c.is_ascii_hexdigit()) {
            return Err(ParadigmError::InvalidKey("Private key contains invalid characters".to_string()));
        }
    ...
    pub fn mnemonic(mnemonic: &str) -> Result<()> {
        ...
        if ![12, 15, 18, 21, 24].contains(&words.len()) {
            return Err(ParadigmError::InvalidKey("Mnemonic must have 12, 15, 18, 21, or 24 words".to_string()));
        }
        
        // Basic word validation - in production, would check against BIP39 wordlist
        for word in words {
            if word.is_empty() || !word.chars().all(|c| c.is_ascii_alphabetic()) {
                return Err(ParadigmError::InvalidKey("Mnemonic contains invalid words".to_string()));
            }
        }
    ...
    pub fn amount(amount_str: &str) -> Result<()> {
        if amount_str.is_empty() {
            return Err(ParadigmError::InvalidAmount("Amount cannot be empty".to_string()));
        }
        
        if amount_str.parse::<f64>().is_err() {
            return Err(ParadigmError::InvalidAmount("Invalid amount format".to_string()));
        }
        
        let value: f64 = amount_str.parse().unwrap();
        if value < 0.0 {
            return Err(ParadigmError::InvalidAmount("Amount cannot be negative".to_string()));
        }
...
pub mod math {
    ...
    pub fn safe_sub(a: u64, b: u64) -> Result<u64> {
        a.checked_sub(b)
            .ok_or_else(|| ParadigmError::Generic("Arithmetic underflow in subtraction".to_string()))
    }
    
    /// Safe multiplication that prevents overflow
    pub fn safe_mul(a: u64, b: u64) -> Result<u64> {
        a.checked_mul(b)
            .ok_or_else(|| ParadigmError::Generic("Arithmetic overflow in multiplication".to_string()))
    }
    ...
    impl TokenBucket {
        ...
        fn refill(&mut self) {
            ...
            if let Ok(elapsed) = now.duration_since(self.last_refill) {
                let tokens_to_add = (elapsed.as_secs() as u32 * self.refill_rate).min(self.capacity - self.tokens);
...
mod tests {
    ...
    fn test_validate_hash() {
        ...
        assert!(validate::hash("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef").is_ok());
    }
