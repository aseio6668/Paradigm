    ...
    fn check_amount_anomaly(
        ...
        if amount_ratio > self.anomaly_threshold || amount_ratio < (1.0 / self.anomaly_threshold) {
            let mut audit = SecurityAudit::new(
                ...
                format!(
                    "Unusual transaction amount detected: {}x normal pattern",
                    amount_ratio
                ),
                vec![transaction.from.clone()],
            );
            audit.add_recommendation(
                "Monitor this address for additional suspicious activity".to_string(),
            );
            Some(audit)
        } else {
            None
...
impl SecurityMonitor {
    ...
    pub fn monitor_transaction(&mut self, transaction: &Transaction) -> Vec<SecurityAudit> {
        ...
        self.anomaly_detector
            .update_pattern(transaction.from.clone(), transaction);
            ...
            if let Ok(threshold_wei) = threshold_str.parse::<u64>() {
                if transaction.value.wei() > threshold_wei {
                    let mut audit = SecurityAudit::new(
                        ...
                        format!(
                            "Large transaction exceeding AML threshold: {} wei",
                    ...
                    audit.add_recommendation(
                        "Conduct enhanced due diligence on transaction parties".to_string(),
                    );
                    audit.add_recommendation(
                        "File suspicious activity report if required".to_string(),
                    );
                    return Some(audit);
                }
            }
    ...
    fn check_ofac_rule(
        ...
        let from_blacklisted = self
            .threat_intelligence
            .blacklisted_addresses
            .contains(&transaction.from);
        let to_blacklisted = transaction
            .to
            .as_ref()
            .map(|addr| {
